...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < sizeNumValues; ++i) {
    ...
    for (int j = 0; j < 5; ++j) {
      ...
      timing = timer.stop();
      fout << "\tBinomial Queue Build: " << std::setprecision(6) << timing << " seconds\n";
      ...
      timing = timer.stop();
      fout << "\tSkew Heap Random Op: " << std::setprecision(6) << timing << " seconds\n";

      // time random operation on binomial queue
      timer.start();
      srand(seed);
      for (int k = 0; k < (n / 10); ++k) {
        type = rand() / RAND_MAX;
      ...
      timing = timer.stop();
      fout << "\tBinomial Queue Random Op: " << std::setprecision(6) << timing << " seconds\n";
