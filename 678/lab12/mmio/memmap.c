...
int main (int argc, char *argv[])
{
  ...
  if (fstat(fdin, &statbuf) < 0) {
    sprintf(buf, "Can't get stats on input file.");
    perror(buf);
    exit(errno);
  }
  ...
  if (write(fdout, "0", 1) < 0) {
    sprintf(buf, "Failure on writing dummy byte.");
    perror(buf);
    exit(errno);
  }
  ...
  if (src < 0) {
    sprintf(buf, "Failure on mapping input file.");
    perror(buf);
    exit(errno);
  }
  ...
  if (dst < 0) {
    sprintf(buf, "Failure on mapping output file.");
    perror(buf);
    exit(errno);
  }
